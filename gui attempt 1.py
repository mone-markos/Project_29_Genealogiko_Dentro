...
def open_edit_person_window():
    ...
    def load_person():
        ...
        if person:
            ...
            first_name_entry.insert(0, person[1])
            last_name_entry.insert(0, person[2])
            birth_date_entry.insert(0, person[3])
            description_entry.insert(0, person[4])
        else:
    ...
    Label(edit_window, text="Περιγραφή:").pack()
...
#!!!!!!!!! isws prepei na xtistei mia tetoia sunarthsh sto Database.py
'''
...
#!!!!!!!!! isws prepei na xtistei mia sunarthsh update_relationship() sto Database.py
...
#!!!!!!!!!!!!!!!!!!!dhmiourgia delete_relationship() sthn db
...
'''
        ...
        if relationship_id.isdigit():  # Έλεγχος εγκυρότητας ID
            Database.delete_relationship(relationship_id)
